            ...
            {serviceCategories.map((category, index) => (
              <div key={index} className="animate-fade-in" style={{ animationDelay: `${index * 0.2}s` }}>
                <div className="flex items-center gap-4 mb-8">
                  <span className="text-4xl font-serif italic text-brand-accent opacity-50">0{index + 1}</span>
                  <h3 className="font-serif text-3xl font-semibold text-white tracking-wide">{category.title}</h3>
            ...
            <div className="bg-brand-accent/10 rounded-3xl p-8 border border-brand-accent/20 sticky top-32">
              ...
              <div className="mt-8 pt-8 border-t border-white/10">
                <p className="text-gray-400 text-sm leading-relaxed">
                  All treatments use premium products to ensure the health and vitality of your hair.
                </p>
              </div>
            </div>
          </div>
